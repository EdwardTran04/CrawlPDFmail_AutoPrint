...
# =====================================
# --- Config ---
$WatchFolder = "./Scripts/In"               # đổi lại nếu bạn muốn thư mục khác
$DoneFolder  = Join-Path $WatchFolder "_printed"
$LogFolder   = "./log"                      # thư mục lưu log
$Printer     = "Canon LBP6030/6040/6018L"          # để $null nếu muốn dùng máy in mặc định
...
function Write-Log {
    ...
    # Ghi vào file log
    $logFile = Join-Path $LogFolder "BatchPrintPDF_$(Get-Date -Format 'yyyy-MM-dd').log"
...
if (Test-Path -LiteralPath $defaultSumatraPath) {
  ...
  Write-Log "Using default SumatraPDF path: $sumatraPath"
}
...
if (-not $sumatraPath) {
  try {
    ...
    if ($cmd) { 
      ...
      Write-Log "Found SumatraPDF in PATH: $sumatraPath"
    }
  } catch {}
}
...
if (-not $sumatraPath -and -not $acroPath) {
  Write-Log "No PDF print app found. Cài SumatraPDF (winget) hoặc đặt portable ở C:\Tools\SumatraPDF.exe." "ERROR"
  return
}

Write-Log "Watch: $WatchFolder"
Write-Log "Printer: $Printer"
Write-Log ("Using: " + ($(if ($sumatraPath) {"SumatraPDF -> $sumatraPath"} else {"Adobe Reader -> $acroPath"})))
...
Write-Log ("Found {0} PDF(s), {1} ready (> {2}s)" -f $all.Count, $ready.Count, [int]$minAge.TotalSeconds)



# --- Print loop ---
Write-Log "Starting print process for $($ready.Count) files" "INFO"
foreach ($f in $ready) {
  try {
    Write-Log ("Printing: {0}" -f $f.Name) "INFO"
...
if ($sumatraPath -and (Test-Path $sumatraPath)) {
    ...
    Write-Log "Using SumatraPDF to print: $($f.Name)" "INFO"
    Start-Process -FilePath $sumatraPath -ArgumentList $arg -NoNewWindow
    Write-Log "SumatraPDF print command executed for: $($f.Name)" "INFO"
}
    elseif ($acroPath -and (Test-Path $acroPath)) {
      # Dự phòng Acrobat Reader
      Write-Log "Using Adobe Reader to print: $($f.Name)" "INFO"
      $arg = if ($Printer) {
      ...
      Write-Log "Adobe Reader print command executed for: $($f.Name)" "INFO"
    ...
    Write-Log ("Successfully printed and moved: {0}" -f $f.Name) "SUCCESS"
  }
  catch {
    Write-Log ("Error printing {0}: {1}" -f $f.FullName, $_.Exception.Message) "ERROR"
    Write-Log ("Error details: {0}" -f ($_.Exception | Format-List -Property * -Force | Out-String)) "ERROR"
  }
}
Write-Log "Print process completed" "INFO"
